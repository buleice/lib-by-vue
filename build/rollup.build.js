const fs = require('fs');
const { formatTypeList, addons, esDir } = require('../config/rollup.build.config');
// const libList = require('../src/lib-list.js');
const { styleOutputPath } = require('../config/index');
const { build } = require('../config/rollup.createConfig');
const { resolve, getAssetsPath } = require('../config/utils');
...
formatTypeList.forEach(({ format, min, suffix } = {}) => {
  Object.keys(libList).forEach((moduleName) => {
    ...
    pkg.push({
      min, format, suffix, moduleName, input, output: `${output}`,
    });
  });
});
